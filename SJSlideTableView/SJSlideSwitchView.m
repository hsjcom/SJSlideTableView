...
@implementation SJSlideSwitchView
...
- (void)layoutSubviews {
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            ...
            _topScrollView.frame = CGRectMake(0, _topScrollView.frame.origin.y, self.bounds.size.width - self.rigthSideButton.bounds.size.width, kHeightOfTopScrollView);
        }
...
- (void)createNameButtons {
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        [button setTag:i + 100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, button.frame.size.height - _shadowImage.size.height, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }
...
- (void)selectNameButton:(UIButton *)sender {
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, sender.frame.size.height - _shadowImage.size.height, sender.frame.size.width, _shadowImage.size.height)];
